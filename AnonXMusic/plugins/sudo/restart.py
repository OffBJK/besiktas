...
async def restart_(_, message):
    ...
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} Alem Müzik Yeniden Başlıyor...\n\nAlem Müzik 15-20 saniye sonra başlayaca...",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
    ...
    await response.edit_text(
        "» Alem Müzik Yeniden Başladı..., Lütfen botun başlaması için birkaç saniye bekleyiniz..."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
